...
class Preprocessing:
    ...
    def preprocess(self):
        ...
        allowed_category_ids = self.filter_csv('./csv/categories.csv')
        ...
        filtered_items_json = self.specified_categories_filter(items_json, allowed_category_ids)
        ...
        if self.autoencoder:
            ...
            ae_train_set, ae_validation_set, ae_test_set = self.train_validation_test_split(
        ...
        dataset_json = self.eliminate_multiple_outfit_instances(d1 + d2 + d3)
        ...
        compatible_tops_accessories, compatible_tops_bottoms, compatible_tops_shoes = \
            self.compatibility_couples_generator(dataset_json, filtered_items_json)
        ...
        if self.autoencoder:
            ...
            compatible_tops_accessories = self.add_not_compatible_items(compatible_tops_accessories, item_ids,
                                                                        item_categories,
                                                                        embeddings)
            compatible_tops_bottoms = self.add_not_compatible_items(compatible_tops_bottoms, item_ids, item_categories,
                                                                    embeddings)
            compatible_tops_shoes = self.add_not_compatible_items(compatible_tops_shoes, item_ids, item_categories,
                                                                  embeddings)
        else:
            compatible_tops_accessories = self.add_not_compatible_items(compatible_tops_accessories,
                                                                        metric=self.not_compatible_items_metric)
            compatible_tops_bottoms = self.add_not_compatible_items(compatible_tops_bottoms,
                                                                    metric=self.not_compatible_items_metric)
            compatible_tops_shoes = self.add_not_compatible_items(compatible_tops_shoes,
                                                                  metric=self.not_compatible_items_metric)

        # Create the json files containing the train, validation and test data for the learning of the GANs
        gan_train_set_ta, gan_validation_set_ta, gan_test_set_ta = self.train_validation_test_split(
            compatible_tops_accessories, test_ratio=0.2)
        gan_train_set_tb, gan_validation_set_tb, gan_test_set_tb = self.train_validation_test_split(
            compatible_tops_bottoms, test_ratio=0.2)
        gan_train_set_ts, gan_validation_set_ts, gan_test_set_ts = self.train_validation_test_split(
    ...
    def filter_csv(self, input_filename):
        allowed_category_ids = {}
        with open(input_filename, 'r', newline='') as file:
            csv_reader = csv.reader(file)
            if self.macrocategories:
    ...
    def specified_categories_filter(self, items, allowed_category_ids):
    ...
    def eliminate_multiple_outfit_instances(self, dataset):
    ...
    def compatibility_couples_generator(self, dataset, filtered_items, outfit_titles=None):
    ...
    def train_validation_test_split(self, dataset, test_ratio, shuffle=False, seed=None):
        if seed:
            shuffle = True
            random.seed(seed)
        if shuffle:
    ...
    def add_not_compatible_items(self, dataset_json, metric=None, item_ids=None, item_categories=None, embeddings=None):
        if metric == 'FID':
            fid = FID(range=255)
...
preprocessing = Preprocessing(macrocategories=False, autoencoder=False, not_compatible_items_metric='FID')
